...
class WGCN(torch.nn.Module):
    def __init__(self,features,r_adj,c_adj,graph_type,n_component,hidden_dim,drop_out, num_bary,layer_num,h,bias=True,**kwargs):
        ...
        if graph_type != 'col':
            ...
            self.u_bary = Bary_layer(self.U_adj_list,self.U_degree,self.h)
        ...
        if graph_type != 'row':
            ...
            self.v_bary = Bary_layer(self.V_adj_list,self.V_degree,self.h)   
...
def model (dataset,n_component,hidden_dim,num_bary,h,drop_out=0,layer_num=4,ce=0,lr=0.001,weight_decay=5e-3,total_iter =10000):
    
    
    features,Odata,Otraining,Otest,r_adj,c_adj,graph_type=load_matrix_data(dataset)
    #print('features',features)
    model = WGCN(Odata*features,r_adj,c_adj,graph_type,n_component,hidden_dim,drop_out,num_bary,layer_num,h).cuda()
...
multi_graph = ['flixster','movielens']
n_component = 25
num_barys = [3,5,7]
hidden_dims= [50]
h_hops = [1,2,3]
for dataset in multi_graph:
    rmse = {}
    print('data:%s, L:%s, h:%s, hidden_dim:%s,n_component:%s'%(dataset,num_barys,h_hops,hidden_dims,n_component))
    results = []
    for num_bary in num_barys:
        for h in h_hops:
            for hidden_dim in hidden_dims:
                result = model (dataset,n_component,hidden_dim,num_bary,h) 
                results.append(result) 
                print('num_bary:%d,h:%d,n_component:%d,hidden_dim:%d,result:%f'%(num_bary,h,n_component,hidden_dim,result))
            rmse[h] = results
    print(rmse)