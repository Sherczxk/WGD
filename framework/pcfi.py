...
def pcfi(edge_index, X, feature_mask, num_iterations=100, missing_type=None, alpha=0.9, beta=1):
    # torch.manual_seed(0)
    # torch.cuda.manual_seed(0)
    # random.seed(0)
    # np.random.seed(0)
    propagation_model = PCFI(num_iterations=num_iterations, alpha = alpha, beta=beta)
    return propagation_model.propagate(x=X, edge_index=edge_index, mask=feature_mask, missing_type=missing_type)
...
class PCFI(torch.nn.Module):
    ...
    def propagate(self, x: Tensor, edge_index: Adj, mask: Tensor, missing_type: str, edge_weight: OptTensor = None) -> Tensor:
        # torch.manual_seed(0)
        # torch.cuda.manual_seed(0)
        ...
        feat_dim = x.shape[1]
        out = x
        if missing_type == 'whole':
            f_n2d = self.compute_f_n2d(edge_index, mask, missing_type)
            ...
            if mask is not None:
                out[mask] = x[mask]
            f_n2d = self.compute_f_n2d(edge_index, mask, missing_type, feat_dim)
    ...
    def compute_f_n2d(self, edge_index, feature_mask, missing_type, feat_dim: OptTensor = None):
