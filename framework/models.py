...
def get_model(model_name, num_features, num_classes, edge_index, args, **kwargs):
...
class GNN(torch.nn.Module):
    def __init__(
        ...
        self.conv_first = get_conv(conv_type, num_features, hidden_dim)
        self.conv_hidden = ModuleList([get_conv(conv_type, hidden_dim, hidden_dim) for _ in range(num_layers - 2)])
        self.conv_out = get_conv(conv_type, hidden_dim, output_dim)
